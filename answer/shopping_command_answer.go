...
func ShoppingCommandAnswer(buffer *[]byte, client *connection.Client) (int, int, error) {
	...
	orm.GormDB.Where("id = ?", data.GetId()).First(&shopOffer)
	...
	if !client.Commander.HasEnoughResource(shopOffer.ResourceID, shopOffer.ResourceNumber*data.GetNumber()) {
	...
	case 2: // packs
		for i, packId := range shopOffer.Effects {
			client.Commander.AddItem(uint32(packId), shopOffer.Number*data.GetNumber())
			response.DropList[i] = &protobuf.DROPINFO{
